...
export async function GET() {
  try {
    ...
    console.log(`🕐 [${now.toISOString()}] Auto-settlement triggered...`)
    ...
    for (const gameType of GAME_TYPES) {
      try {
        ...
        for (const round of pendingRounds) {
          try {
            console.log(`📊 Auto-settling round ${round._id} for ${gameType}`)
            ...
            console.log(`✅ Successfully auto-settled round ${round._id}`)
            
          } catch (roundError) {
            console.error(`❌ Failed to auto-settle round ${round._id}:`, roundError)
            totalErrors++
            results.push({
              roundId: round._id,
              gameType,
    ...
    console.log(`🏁 Auto-settlement complete: ${totalSettled} rounds settled, ${totalErrors} errors`)
